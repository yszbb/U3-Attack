...
concept_embeds_weights = torch.load("weights/concept_embeds_weights.pt").to(device)  # ! threshold

feature_extractor = torch.load("weights/feature_extractor.pt")
special_care_embeds = torch.load("weights/special_care_embeds.pt").to(device)
special_care_embeds_weights = torch.load("weights/special_care_weights.pt").to(device)  # ! threshold
...
def compute_grad(cur_mask, cur_masked_image, patch, prompt, pipe_inpaint, x, y, **kwargs):
    ...
    patch_new = patch.clone().half().to(device)
    ...
    for ind in range(image_gen.shape[0]):
        ...
        # safety_checker_input = feature_extractor(numpy_to_pil(image_nat_num), return_tensors="pt").to(device)
...
def compute_grad_pipeline(cur_mask, cur_masked_image, patch, prompt, pipe_inpaint, x, y, **kwargs):
    ...
    patch = patch.clone().half().to(device)
...
def super_l2(cur_mask, cur_masked_image, patch, prompt, step_size, iters, eps, clamp_min, clamp_max, pipe_inpaint, x, y,
    ...
    loss_best = 1000.0
    patch_best = None
    patch_adv = patch.clone().to(device)
    #  计算梯度更新patch
